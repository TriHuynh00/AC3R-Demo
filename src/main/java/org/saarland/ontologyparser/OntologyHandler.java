...
public class OntologyHandler {
	...
	public void readOntology() throws OWLOntologyCreationException {
		...
		for (OWLClass owlClass : accidentOntology.getClassesInSignature()) {
			...
			// reasoner.getSubClasses(owlClass,false).getFlattened();
			...
			if (leafLvlStrIndex > 0) {
				...
				for (OWLNamedIndividual keyWord : individualsList) {
					...
					// ConsoleLogger.print('d',"Class=>" +
					// owlClassFullName.substring(owlClassFullName.indexOf("#")
					// + 1, leafLvlStrIndex));
					accidentConcept.setConceptGroup(
							owlClassFullName.substring(owlClassFullName.indexOf("#") + 1, leafLvlStrIndex));

					// ConsoleLogger.print('d',"Obj Prop " +
					// factory.getOWLObjectProperty("isVehicleType",
					// pm).getIndividualsInSignature());
					// ConsoleLogger.print('d',"ParentClass=>" +
					// reasoner.getSuperClasses(owlClass,
					// false).getFlattened().toString());
					...
					Set<OWLDataPropertyAssertionAxiom> dataProperties = accidentOntology
							.getDataPropertyAssertionAxioms(keyWord);
					...
					if (dataProperties.size() > 0) {
						...
						for (OWLDataPropertyAssertionAxiom prop : dataProperties) {
							...
							// ConsoleLogger.print('d',propName + " " +
							// prop.getObject().getLiteral());
		...
		// ConsoleLogger.print('d',"Concept group: " +
		// concept.getConceptGroup());
		// ConsoleLogger.print('d',"Category: " + concept.getCategory());
		// ConsoleLogger.print('d',"Input Group: " + concept.getInputGroup());
		// ConsoleLogger.print('d',"Data Properties " +
		// concept.getDataProperties());
		// }
	...
	public AccidentConcept findConcept(String keyword) {
		...
		ConsoleLogger.print('e', ">>> Cannot find concept " + keyword);
		return null;
	}
