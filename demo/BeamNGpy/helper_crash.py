...
def getV1BeamNGCoordinaes(total_distance_v1, width, road_a):
    # global road_a
...
def getV2BeamNGCoordinaes(total_distance_v2, width, road_b):
    # global  road_b
...
def angle_between(v1, v2, direction=None):
    """ Returns the angle in radians between vectors 'v1' and 'v2' """
    v1_u = unit_vector(v1)
    v2_u = unit_vector(v2)

    weight = -1 if direction == 'E' else 1
    return weight * np.degrees(np.arccos(np.clip(np.dot(v1_u, v2_u), -1.0, 1.0)))
